    ...
    <>
      ...
      >
        {imgFile.url.trim().length === 0 && (
          <p className="text-center border-solid border border-zinc-300 w-80 h-80 bg-white">
            No Image
          </p>
        )}
        {imgFile.url.trim().length > 0 && (
          <img
            className="text-center border-solid border border-zinc-300 rounded w-80 h-80 bg-white"
            src={imgFile.url}
            alt={`${imgFile.url}`}
          />
        )}
        ...
        <input
          className="p-1.5 mb-2 w-80 text-xl rounded"
          placeholder="이름 수정"
          name="name"
        ...
        />
        <input
          className="p-1.5 w-80 text-xl rounded"
          placeholder="비밀번호 수정"
          name="password"
      ...
      <div className="flex flex-col items-center mt-10">
        <button
          className="bg-gray p-1 w-80 mb-2 text-2xl rounded"
          onClick={cancelHandler}
        >
          취소
        </button>
        <button
          className="bg-green w-80 p-1 mb-2 sm:mb-6 text-2xl rounded"
